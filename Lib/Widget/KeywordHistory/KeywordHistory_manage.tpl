...
<table class="layui-table">
  	<thead>
	    ...
	    <tr>
			...
			<!-- 排名 -->
			<td><if condition="$vo['rank'] GT 0 AND $vo['rank'] ELT 10">
				<span style="color: red">{$vo['rank']}</span>
				<else/>
				{$vo['rank']|default='100+'}
				</if></td>
			<td>
				<a href="javascript:void(0);" onclick="open_layer('指定排名','{:U('setRankPage')}/id/{$vo['id']}/rank/{$vo['rank']}/original/{$vo['rank_original']}/day/{$vo['createtime']|format_date}?returnUrl={$returnUrl|urlencode}',500,350)" class="layui-btn layui-btn-mini">指定排名</a>
			</td>
