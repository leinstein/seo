...
<table class="layui-table">
	...
	<if condition="$vo['site']|count gt 0">
		...
		<else />
				<td rowspan="1">{$vo['epdir']['seller_manager']['username']}<notempty name="vo['epdir']['seller_manager']['truename']">（{$vo['epdir']['seller_manager']['truename']}）</notempty></td>
				</if>
	...
	<if condition="$vo['site']|count gt 0">
		<td rowspan="{$vo['site']|count}">
			{$vo['epdir']['seller']['username']}
			<notempty name="vo['epdir']['seller']['truename']">（{$vo['epdir']['seller']['truename']}）</notempty>
		</td>
		<else />
		<td rowspan="1">
			{$vo['epdir']['seller']['username']}
	...
	<if condition="$vo['site']|count gt 0">
		<td rowspan="{$vo['site']|count}">
			<a href="{:U('Remark/index')}/productid/1/objecttype/user/objectid/{$vo['id']}/touserid/{$vo['id']}/tousername/{$vo['username']}&returnUrl={$returnUrl}" class="layui-btn layui-btn-mini">查看日志</a>
		</td>
	<else />
		<td rowspan="1">
			<a href="{:U('Remark/index')}/productid/1/objecttype/user/objectid/{$vo['id']}/touserid/{$vo['id']}/tousername/{$vo['username']}&returnUrl={$returnUrl}" class="layui-btn layui-btn-mini">查看日志</a>
	...
	<if condition="$vo['site']|count gt 0">
		<td rowspan="{$vo['site']|count}">
			<eq name="vo.can_edit" value="1">
				<a class="layui-btn layui-btn-mini"  href="javascript:;" onclick="open_layer('修改子用户','{:U('UserManagement/updatePage')}/id/{$vo['id']}/usertype/sub?returnUrl={$returnUrl}','50%')" >修改</a>
				<a class="layui-btn layui-btn-mini"  href="javascript:;" onclick="open_layer('修改子用户密码','{:U('UserManagement/updatePasswordPage')}/id/{$vo['id']}/usertype/sub?returnUrl={$returnUrl}','50%')" >密码</a>
