...
<table class="layui-table">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<eq name="vo.isopen_subagent" value="1">
					<gt name="vo['sub_agent_num']" value="0">
						<a href="javascript:;" onclick="open_layer('查看子代理','{:U('/Manage/UserManagement/sub_agent_list')}/id/{$vo['id']}','100%')" >{$vo['sub_agent_num']}</a>
					<else/>
						0
					</gt>
				<else/>
			...
			<!--管理-->
			<td>
				<a href="javascript:;" class="layui-btn layui-btn-danger layui-btn-mini" onclick="open_layer('一级代理商充值','{:U('rechargePage')}/id/{$vo['id']}&returnUrl={$returnUrl}',500,460)" >充值或退款</a>		
