...
class SystemAction extends BaseAction
{
    ...
    public function bpupdate()
    {
        ...
        $result                     = $model->updateAgentOEMConf($_POST, $t);
        if ($result)
        {
            $this->success('更新成功！', U('bpxdlist'));
        }
        else
        {
            $this->error('下单失败，原因' . $model->getError());
        }
    }
     ...
     */
    public function bpcheck()
    {
        $model                      = D($this->modelName);
        $_POST['attachmenttype'][0] = 'login_page_image';
        $_POST['attachmenttype'][1] = 'loginpage_logo_image';
        $result                     = $model->updatecheckAgentOEMConf($_POST);
        if ($result)
        {
            $this->success('更新成功！', U('bpxdlist'));
        }
    ...
    public function update()
    {
        ...
        if ($result)
        {
            $this->success('修改系统配置成功！', U('bpindex'));
        }
        else
        {
