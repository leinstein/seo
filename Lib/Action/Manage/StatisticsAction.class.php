...
class StatisticsAction extends BaseAction {
	...
	public function index() {
		...
		$stankeywordNum = $modelStandardfee -> getTodayNum();
		$page['stankeywordNum'] = $stankeywordNum;
		...
		$page['compliance_rate'] = round( $stankeywordNum / $purchasedKeywordNum ,2) * 100 .'%';
		// 获取本月消费
		...
		// 获取上个月的消费记录
		$consumerdetails = $modelStandardfee -> getConsdetailsForLastMonth();
		foreach ($consumerdetails as $vo){
			$days[]	 				= $vo['day'];
			$cons_last_month[] 		= $vo['consumption'];
		}
		$page['days'] = json_encode($days) ;
		$page['consumptions_last_month'] = json_encode($cons_last_month) ;
		
		// 获取本月的消费统计
		$consumerdetails_this = $modelStandardfee -> getConsdetailsForThisMonth();
		foreach ($consumerdetails_this as $vo){
			$cons_this_month[] =$vo['consumption'];
		}
		$page['consumptions_this_month'] = json_encode($cons_this_month) ;
		
		// 获取上个月的消费记录
		$standardNumsLast = $modelStandardfee -> getStandardNumForLastMonth();
		foreach ($standardNumsLast as $vo){
			$standard_num_last_month[] 	= $vo['num'];
			$standard_rate_last_month[] = $vo['rate'];
		}
		$page['standard_num_last_month'] = json_encode($standard_num_last_month) ;
		$page['standard_rate_last_month'] = json_encode($standard_rate_last_month) ;
		
		// 获取上个月的消费记录
		$standardNumsThis = $modelStandardfee -> getStandardNumForThisMonth();
		foreach ($standardNumsThis as $vo){
			$standard_num_this_month[] 	= $vo['num'];
			$standard_rate_this_month[] = $vo['rate'];
		}
		$page['standard_num_this_month'] = json_encode($standard_num_this_month) ;
		$page['standard_rate_this_month'] = json_encode($standard_rate_this_month) ;
