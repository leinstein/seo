...
class SiteAction extends BaseAction {
	...
	function effect(){
		...
		if( $this -> isMobile ){
			...
			$page['list']  = $model -> getEffectForSub( $map );
			// 高亮关键词
			foreach ($page['list']['data'] as &$vo ){
				$vo['sitename'] = str_ireplace($_GET['keyword'],'<span style="color: #c00">'.$_GET['keyword'] .'</span>' , $vo['sitename']);
	...
	function history(){
		...
		//获得查询结果，传值到模板输出查询的结果
		$page['list'] = $model->getHistory( $_GET['id'], $_GET['detecttime'],'sub' );
	...
	public function insert(){
		...
		if( $result ){
			$this-> success('新增站点成功！' , U('index'),false,true);
		}else{
			$this-> error('新增站点失败！');
		}
	}
