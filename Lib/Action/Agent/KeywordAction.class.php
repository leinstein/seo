	...
	function effect(){
		...
		if( $this -> isMobile ){
			...
			$me = $this -> loginUserInfo;
		 	if( $me['id'] == 245 ){
		 		$page = $model -> getEffect($map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page']  ,'all');
		 	}else{
				switch (  $me ['usertype']  ) {

					case 'agent':// 一级代理

						$page  = $model -> getEffectForAgent( $map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page'] );

						break;

					case 'sales_manager':// 销售经理：获取自己的客户或者员工的客户

						$page  = $model -> getEffectForSaleManager( $map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page'] );

						break;
					case 'seller':// 销售：获取自己的客户

						$page  = $model -> getEffectForSeller( $map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page'] );

						break;
					case 'customer_manager':// 客服经理：获取自己的客户或者员工的客户

						$page  = $model -> getEffectForCustomerManager( $map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page'] );

						break;
					case 'customer':// 客服：获取自己的客户

						$page  = $model -> getEffectForCustomer( $map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page'] );
			...
			//添加默认排序参数-按照天时间排序
			$querytools->addDefOrder('id desc');
			$querytools->addDefOrder('latestranking = 0,latestranking asc');
			//添加排序参数
			if( $_GET['ord'] == 'latestranking'){
				$querytools->addOrder('latestranking','latestranking!=0 desc,latestranking,id desc');
			}else{
			...
			if( $me['id'] == 245 || $me['id'] == 32) {
				$data['record'] = $model -> getEffect($map, $fields, $querytools->getOrder(), $querytools->getPageparam() ,$_GET['num_per_page']  ,'all');
			}else{
                switch (  $me ['usertype']  ) {
					case 'agent':// 一级代理
