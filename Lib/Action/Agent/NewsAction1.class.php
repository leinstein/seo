...
class NewsAction extends BaseAction {
    ...
    */
    ...
    public function index(){
        ...
        switch ($me['usertype']) {
        	case 'agent':
        		//获得查询结果
        		$data =   $model -> getListByOperationuser() ;
        		break;
        			
        	default:
        		// 如果该子用户的代理商已经开通了OEM权限
        		if( $me['oem_config']['id']){
        			$data =   $model -> getListByAgentuser( $me['oem_config']['userid'] ) ;
        		}else{
        			$data =   $model -> getListByOperationuser() ;
        		}
        		break;
    ...
    public function publist(){
    	...
    	//获得查询结果
    	$page['list'] =   $model -> getListByAgentuser() ;
    	// 输出模板
    	$this -> assign( $page);
   ...
   /*  public function index(){
        ...
        $model =  D( $this -> modelName );
        $me = $this -> loginUserInfo;
        $map['pubuserid'] 	= $me['pid'];
        $map['status'] 		= 1;
		//获得查询结果
		$data =   $model -> queryRecordEx($map, $fields, $order,  $url_param) ;
		// 输出模板
		$this -> assign($model -> getModelName() . 's', $data);
 ...
 */
    ...
    public function insert() {
        ...
        if ($result) {
            //显示提示
            $this -> assign('jumpUrl', __URL__ . '/' . 'publist');
     ...
     */
    public function maintainPage() {

        //初始化模型
        $model = D($this -> modelName);
       
        $map['newstype'] = $_GET['newstype'];
        $map['pubuserid'] = $this -> loginUserId;
        $map['status'] = 1;
    ...
    public function updatePage() {
    	...
    	//查询单条记录
    	$page['data'] = $model -> selectOne($this -> getPKMap($model));
    ...
    public function update() {
        ...
        if ($result) {
           ...
           /*  $this->assign('jumpUrl', __URL__);
            ...
            $this->success ( '修改成功！', U ( 'publist'), false, true);
        } else {
            $this->error($model->getError());
        }
    }
