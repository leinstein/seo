...
class ProductModel extends BaseModel{
	 ...
	 * 用户表名称
	 */
	protected $trueTableName = 'ts_sys_product';
...
/**
	...
	function selectOne( $map, $field  ){
		...
		if( $data ){
			...
			// 解析module_name数组
			if( $data['module_name']) {
				$data['module_name_arra'] = json_decode( $data['module_name'] ,true );
			}
		}
		return $data;
	}
	...
	public function queryRecordAll($map, $field = null, $order = null, $relation = false, $maxCount = 10000) {
		...
		foreach( $list as &$vo ){
			...
			// 解析module_name数组
			if( $vo['module_name']) {
				$vo['module_name_arra'] = json_decode( $vo['module_name'] ,true );
			}
	...
	function getProducts() {
		...
		return $this -> queryRecordAll(null,'id,product_guid,product_code,product_name');
	 ...
	 * 获取产品
	 */
	function getProductsCodeset() {
		
		$list =  $this -> getProducts();

		foreach ($list as $key => $vo) {
			$codeSet[$vo['id']] = $vo['product_name'];
		}
		return $codeSet;
	}
}
