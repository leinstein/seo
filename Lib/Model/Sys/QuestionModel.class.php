...
class QuestionModel extends BaseModel{
    ...
    function _initialize() {
        ...
        $this -> trueTableName= C('DB_PREFIX') . 'sys_question';
    ...
    public function queryRecordAll($map, $field = null, $order = null, $relation = false, $maxCount = 10000) {
    	...
    	foreach( $list as &$vo ){
    		// 如果是子用户，并且该用户的代理商已经设置了产品的名称，那么需要替换问题库的优站宝相关字段
    		if( $me['usertype'] == 'sub' && $me['oem_config']['product_name']){
    			$vo['questtitle'] = str_replace('优站宝','【'.$me['oem_config']['product_name'].'】',$vo['questtitle']);
    			$vo['questcontent'] = str_replace('优站宝','【'.$me['oem_config']['product_name'].'】',$vo['questcontent']);
    		}
    	}
    	//返回记录集
    	return $list;
