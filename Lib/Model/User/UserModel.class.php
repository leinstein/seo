...
class UserModel extends BaseModel{
	 ...
	 * 用户信息的session名称
	 */
	protected $login_usertype_session 	= 'LoginUserType';
	
	
	/**
	 * 用户信息的session名称
	 */
	protected $login_userepid_session 	= 'LoginEpid';
	...
	function _initialize() {
		...
		$this ->login_userepid_session 	= C('SESSION_PREFIX') .'LoginEpid';
		$this ->login_usertype_session 	= C('SESSION_PREFIX') .'LoginUserType'; 
	}
	...
	function selectOne( $map,$field ){
		...
		if( $data ){	
			...
			if( $data['epid'] && $data['usergroup'] != 'Manage'){
	        	...
	        	$epdir = $model_epdir -> selectOne( array('id' => $data['epid']),'id,epname,seller,operationer,customer,product,isopen_oem,isopen_subagent,is_recharge_limit');
	        	$data['epdir']  		= $epdir;
	        	$data['epname']  		= $epdir['epname'];
	        	$data['isopen_oem']  	= $epdir['isopen_oem'];
	        	$data['isopen_subagent']  = $epdir['isopen_subagent'];
	        	$data['is_recharge_limit']  = $epdir['is_recharge_limit'];
				...
				if( $data['usergroup'] =='Agent' && $data['isopen_oem'] ){
					...
					if(  !in_array($host,C('HostOptions')) ){
						...
						if( $data_oem['show_mode'] == 'customer'){
							...
							$oem_config['customer'] = $data_oem['customer'];
							$oem_config['logo_image_arr']['fileid'] = $data_oem['logo_image_arr']['fileid'];
	...
	function transList( &$list ){
		...
		if( $epids ){
			...
			$epdirs = $model_epdir -> queryRecordAll( $map_ep/*,'id,epname,seller,operationer,customer,product,isopen_oem,isopen_subagent,is_recharge_limit'*/);
		}
		// 加载用户类型
		...
		foreach( $list as &$vo_user ){
			...
			$vo_user['is_recharge_limit']  = $epdir[0]['is_recharge_limit'];
	 ...
	 * @return string 当前登录的用户编号
	 */
	public function getLoginEpid( ) {
		return $_SESSION[$this->login_userepid_session];
	}
	...
	public function getLoginUserType( ) {
		return $_SESSION[$this->login_usertype_session];
	}
	...
	public function setUserSession( $userid, $username, $userinfo ) {
		...
		if( $userinfo ){
			$prefix  = strtoupper( $userinfo['role_info']['rolegroup'] );
			if( !$prefix ){
				$prefix  = strtoupper( $userinfo['usergroup']);
			}
			...
			$session_prefix = $prefix .'_SESSION_';
			/* //按照配置重写设置session变量名
			...
			$this ->login_userepid_session 	= $session_prefix .'LoginEpid';
			$this ->login_usertype_session 	= $session_prefix .'LoginUserType';
		...
		$_SESSION[$this->login_userepid_session] 	=  $userinfo['epid'] ;
		$_SESSION[$this->login_usertype_session] 	=  $userinfo['usertype'] ;
		...
		session(array('name'=>$this->login_userepid_session,'expire'=>60));
		session(array('name'=>$this->login_usertype_session,'expire'=>60));
...
// 		}
	...
	function getUsersForSale( $loginUserInfo ){
		...
		if( $staffids ){
			$model_epdir 	= D ( 'Sys/Epdir' );
			$map_epdir['seller'] 	= array('IN' , $staffids);
			$map_epdir['status'] 	= 1;
			$epdirs = $model_epdir -> queryRecordAll( $map_epdir,'id' );
			foreach ($epdirs as $vo_ep) {
				$epids[] = $vo_ep['id'];
			}
			if($epids){
	...
	function getAgentBySeller( $userids ){
		...
		if( $userids ){
			...
			if( $epids ){
				...
				$map['status'] 		= 1;
				$list = $this -> queryRecordAll($map,'id,epid,username,truename,mobileno,telephone,QQnumber,contact,epname,usertype,usertype_desc,createtime' );
			}
		}
	 ...
	 * @param string $userid
	 */
	function getUsersBySellerManager( $userid ){
	 ...
	 * @param string $userid
	 */
	function getUsersByCustomerManager( $userid ){
		...
		$map_ep['customer_manager'] = $userid;
		$map_ep['status'] = 1;
		$epdirs  =  $model_epdir -> queryRecordAll( $map_ep);
		foreach ($epdirs as $vo ){
			$epids[] = $vo['id'];
	 ...
	 * @return array 的用户信息
	 */
	static public function getUserinfoByWx( $wxopenid, $full = true ) {
		$map['openid'] = $wxopenid;
		//update By Richer 于20161125 由于解除绑定的时候，不删除当前绑定信息
		$map['status'] = 1;
	
		$wxuserinfo = M("Sys_wxbindinfo")->where($map)->find();
		if( $wxuserinfo ){
			$userid = $wxuserinfo['userid'];
			$userinfo = D('User/User') -> getUserinfoById($userid, $full);
			$userinfo['wechatinfo'] = $wxuserinfo;
			return $userinfo;
		}else
			return false;
	}
	 ...
	 * 根据用户编号获得用户信息
	 *
	 * @param $userId 用户编号，可以标示一个用户
	 *
	 * @return array用户信息
	 */
	public function getUserinfoById( $userId, $full = true) {
		$map["id"] = $userId? $userId : $this->getLoginUserId();
		return $this->selectOne($map, $full);
	}
	...
	public function get_sales_managers( $epid ) {
		if( !$epid ){
			$epid = $this -> getloginUserEpid();
		}
		$map['epid'] 		= $epid;
		$map['usertype'] 	= 'sales_manager';
		$map['status'] 		= 1;
		$list = $this -> queryRecordAll( $map ,'id,username,truename');
    ...
    public function updateAgentOEMConf($postData)
    {
...
//show($postData);die;
        ...
        if ($postData['id'])
        {
            $result = $model_OEM->updateRecord($postData);
        }
        else
        {
            $result = $model_OEM->addRecord($postData);
        }
        return $result;
    }
