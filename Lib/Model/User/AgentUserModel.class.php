...
class AgentUserModel extends UserModel{
	 ...
	 * 获取用户今天的全部消耗
	 * 1、获取该代理的全部子用户
	 * 2、从达标消费表中获取ownuserid 等于当前子用户id的今日消费记录
	 *
	 * @return mixed 查询结果
	 */
	public function getTodayConsumption() {
		...
		foreach ($members_list as $vo ){
			$memberids[] = $vo['id'];
		}
		$memberids = array_unique( $memberids );
		// 从达标扣费表中获取消费金额
		if( $memberids ){
			$map['ownuserid'] = array('IN',$memberids);
			$today_consumption = D('Biz/Standardfee') -> getTodayFee( $map );
		}
	 ...
	 * @return mixed 查询结果
	 */
	public function getMembersList( $id ) {
		if( !$id ){
