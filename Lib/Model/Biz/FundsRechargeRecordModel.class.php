...
class FundsRechargeRecordModel extends BaseModel{
	...
	public function queryRecordEx($map, $fields, $order = null,  $url_param = '', $num_per_page = 20) {
		...
		if( $productids  ){
			$map_product['id'] = array('IN',$productids );
			$products = D('Sys/Product') -> queryRecordAll( $map_product );
		}
		unset( $vo );
		foreach( $list['data'] as  $key => &$vo ){
			//计算序号
	 ...
	 * @param float $amount
	 */
	function addRecord(  $userid, $usertype,$amount, $productid,$writepriv, $puserid, $remarks ){
		$data['userid'] 	= $userid;
		if ( !$puserid ){
	 ...
	 * @param int $id
	 */
	function getRechargeRecords( $userid ){
		...
		$list = $this -> queryRecordEx( $map,null,'regtime desc');
		foreach ($list['data'] as &$vo){
			$vo['username'] = $me['username'];
			$vo['usertype'] = $me['usertype'];
			$vo['usertype_desc'] = $me['usertype_desc'];
			$vo['truename'] = $me['truename'];
			$vo['epname'] 	= $me['epname'];
		}
