...
class FinanceModel extends BaseModel{
	...
	function recharge( $postData ){
		...
		// dump( $_POST['usertype'] );exit;
		switch ( $_POST['usertype'] ) {
			case 'agent':
				$result_funds = $model_funds -> rechargeForAgentUser( $_POST['id'], $_POST['usertype'], $_POST['amount'], $_POST['productid']);
			break;
			case 'sub':
				$result_funds = $model_funds -> rechargeForSubUser( $_POST['id'], $_POST['usertype'], $_POST['amount'], $_POST['productid']);
				break;
			case 'agent2':
				$result_funds = $model_funds -> rechargeForSubAgent( $_POST['id'], $_POST['usertype'], $_POST['amount'], $_POST['productid']);
				break;
			default:
				;
			break;
		...
		if( $result_funds){
            ...
            $model_recharge -> addRecord( $_POST['id'], $_POST['usertype'], $_POST['amount'], $_POST['productid'], $_POST['writepriv']);
		}else {
	...
	function getRechargeRecords(){
		// 充值记录模型
		$model_recharge = D('Biz/FundsRechargeRecord');
		return $model_recharge -> getRechargeRecords();
	}
	 ...
	 * 获取子用户的资金池信息
	 */
	function getFundsPoolSub( $userid ){
		// 资金账户模型
		$model_funds 	= D('Biz/Funds');
		$model_standardfee 	= D('Biz/Standardfee');
		if( !$userid ){
		...
		$map2['ownuserid'] = $userid;
		$map2['status'] = 1;
		$consumptionfunds = $model_standardfee  -> where($map2) ->  sum('price');
		$balancefunds 	= $data['totalfunds'] - $consumptionfunds;
		if(  $data['freezefunds']  > 0 ){
			$availablefunds = $data['totalfunds'] - $data['initfreezefunds'] ;
		}else{
			$availablefunds = $balancefunds;
		}
	...
	function getFundsPoolAgent( $userid ){
		...
		//	$data['availablefunds'] 	= $availablefunds;
			...
			$data['consumptionfunds'] 	= $model_standardfee  -> where( $map_standardfee ) ->  sum('price');
			if( $data['consumptionfunds'] > $initfreezefunds){
				$data['availablefunds'] 	= $data['totalfunds'] - $data['consumptionfunds'];
			}else{
				$data['availablefunds'] 	= $data['totalfunds'] - $initfreezefunds;
			}
			
		}else{
			// 资金余额
			$data['balancefunds'] 		= $data['totalfunds'];
			// 可用余额
			$data['availablefunds'] 	= $data['totalfunds'];;
			// 冻结金额
			$data['freezefunds'] 		= 0;
			// 冻结初始金额
			$data['initfreezefunds'] 	= 0;
		}
		return $data;
	}
	...
	function refunds( $postData ){
		...
		$puserid 	= $postData['puserid'];
		// 获取资金池的信息
		$map_funds['productid'] = $productid;
		$map_funds['userid'] 	= $userid;
		$map_funds['status'] 	= 1;
		$funds = $model_funds -> where( $map_funds ) -> find();
		/* $totalfunds 	= $funds['totalfunds'];
		$balancefunds 	= $funds['balancefunds'];
		$availablefunds = $funds['availablefunds']; */
		
		// 组合更新数据信息
		$data['id'] 			= $funds['id'];
		$data['totalfunds'] 	= $funds['totalfunds'] - $amount;
		$data['balancefunds'] 	= $funds['balancefunds'] - $amount;
		$data['availablefunds'] = $funds['availablefunds'] - $amount;
		$data['freezefunds'] 	= $funds['freezefunds'];
		if ( $data['freezefunds'] < 0 ){
			$data['freezefunds'] = 0 ;
		}
		$result = $model_funds -> update( $data );
		//dump($model_funds ->  _sql());
		if( $result ){
			// 更新上级用户的资金池
			// 获取资金池的信息
			$map_funds_p['userid'] 	= $puserid;
			$map_funds_p['status'] 	= 1;
			$funds_p = $model_funds -> where( $map_funds_p ) -> find();
			$data_p['id'] 				= $funds_p['id'];
			$data_p['balancefunds'] 	= $funds_p['balancefunds'] + $amount;
			$data_p['availablefunds'] 	= $funds_p['availablefunds'] + $amount;
			$model_funds -> update( $data_p );
			//dump($model_funds ->  _sql());
			// 向充值记录表中增加一条记录
			$model_recharge -> addRecord( $userid, $postData['usertype'], '-'.$amount, $productid , $puserid, $postData['remarks']);
			//dump($model_recharge ->  _sql());
			/* $record['userid'] 		= $userid;
			$record['amount'] 		= '-'.$amount;
			$record['usertype'] 	= $postData['usertype'];
