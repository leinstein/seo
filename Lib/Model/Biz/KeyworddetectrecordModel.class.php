...
class KeyworddetectrecordModel extends BaseModel{
	...
	function _initialize() {
		//执行父类构造函数
		parent::_initialize();
		$this -> trueTableName= C('DB_PREFIX') . 'keyworddetectrecord';
	}
	 ...
	 */
	function addRecord( $keywordid, $keyword, $website, $searchengine, $createuserid ){
		// 判断该关键词今天是否已经建成过了
	 ...
	 *
	 * @param int $rank 排名
	 * @param int $rank_real 真实片面
	 * @param int $keywordid 关键词id
	 * @param string $keyword 关键词信息
	 * @param string $website 网址信息
	 * @param string $searchengine 搜索引擎
	 * @param string $createuserid 用户id
	 * @return mixed
	 */
	function updateRecord( $rank,$rank_real,$keywordid, $keyword, $website, $searchengine, $createuserid ){
		...
		if( !$data){
			$data['rank']			= $rank;
			$data['rank_original']	= $rank;
			$data['rank_real']		= $rank_real;
			$data['ownuserid'] 		= $createuserid;
			$data['keywordid'] 		= $keywordid;
			$data['keyword'] 		= $keyword;
			$data['website'] 		= $website;
		...
		Log::write("------------------------------ 更新关键词检测表记录：". $this -> _sql());
		return $result;
	}
	 ...
	 * 批量新增
	 */
	function addRecords( $list,$app_id ){
		foreach ( $list as $vo ){
			
			// 判断是否存在，如果存在将status改为1
			$map['keywordid'] 	= $vo['id'];
			$map['createtime'] 	= array('LIKE',date('Y-m-d').'%');
			...
			// 如果新增成功才进行推送消息处理，并且该关键词是还未检测过的关键词
			//if(  $vo['is_detect'] != 1 ){
				$data['ownuserid'] 		= $vo['createuserid'];
