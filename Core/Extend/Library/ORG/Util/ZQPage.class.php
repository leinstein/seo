...
class ZQPage extends Think {
    ...
    protected $rollPage;
	// 分页显示定制
	protected $config = array('rows_info'=>'总计:','pages_info'=>'当前:','prev'=>'上一页','next'=>'下一页','up'=>'快退','down'=>'快进','first'=>'首页','last'=>'尾页','theme'=>'%rows_info% %totalRows% %pages_info% %nowPage%/%totalPage% &nbsp; %first% %prePage% %upPage% %linkPage% %downPage% %nextPage% %end%');

	public function __construct($totalRows,$listRows,$style=0,$language=0,$type=1,$parameter=''){
	...
	public function show(){		
		...
		if($this->language==2){
			$this->config['rows_info']='总计:';
			$this->config['pages_info']='当前:';
		}
		if($this->language==3){
			$this->config['rows_info']='Total:';
			$this->config['pages_info']='Page:';
