...
class TagLibHtml extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'select'      => array('attr'=>'name,options,values,output,multiple,id,size,first,change,selected,dblclick,lay_verify,lay_filter,readonly,lay_search','close'=>0),
    ...
    public function _select($attr) {
		...
		$onchange	= $tag['change'];
		$lay_verify	= $tag['lay_verify'];
		$lay_filter	= $tag['lay_filter'];
		$readonly	= $tag['readonly'];
		$lay_search	= $tag['lay_search'];
        if(!empty($multiple)) {
            $parseStr = '<select id="'.$id.'" name="'.$name.'" ondblclick="'.$ondblclick.'" onchange="'.$onchange.'" multiple="multiple" class="'.$style.'" size="'.$size.'" lay-verify="'.$lay_verify.'" lay-filter="'.$lay_verify.'"  readonly="'.$readonly.'" >';
            if( $lay_search == true ){
            	$parseStr .= 'lay-search';
            }
            $parseStr .= '>';
        	...
        	$parseStr = '<select id="'.$id.'" name="'.$name.'" onchange="'.$onchange.'" ondblclick="'.$ondblclick.'" class="'.$style.'" lay-verify="'.$lay_verify.'" lay-filter="'.$lay_verify.'" readonly="'.$readonly.'" ';
        	if( $lay_search == true ){
        		$parseStr .= 'lay-search';
        	}
        	$parseStr .= '>';
    ...
    public function _radio($attr) {
        ...
        foreach($radios as $key=>$val) {
            if($checked == $key ) {
                $parseStr .= '<input type="radio" checked="checked" name="'.$name.'[]" value="'.$key.'" title="'.$key.'">'.$val.$separator;
            }else {
                $parseStr .= '<input type="radio" name="'.$name.'[]" value="'.$key.'" title="'.$key.'">'.$val.$separator;
            }
