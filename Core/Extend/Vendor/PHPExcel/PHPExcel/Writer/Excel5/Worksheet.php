...
//                  if ($type == 0x03) { // list type
                    if ($type == 0x03 && preg_match('/^\".*\"$/', $formula1)) { // list type
                   // <<< Added by S.Sakai
						$formula1 = str_replace(',', chr(0), $formula1);
					}
					$this->_parser->parse($formula1);
					 // >>> Added by S.Sakai
                    if (is_array($this->_parser->_parse_tree)
                            && strtoupper($this->_parser->_parse_tree['value']) == 'INDIRECT') {
                        $this->_parser->_parse_tree['force_reference'] = true;
                    }
                    // <<< Added by S.Sakai
