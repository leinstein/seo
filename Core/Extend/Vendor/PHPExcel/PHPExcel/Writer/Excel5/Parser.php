...
//  function _convertFunction($token, $num_args)
    function _convertFunction($token, $num_args, $force_reference)
    // <<< Added by S.Sakai
	{
...
//            return pack("CCv", $this->ptg['ptgFuncVarV'], $num_args, $this->_functions[$token][0]);
            if ($force_reference) {
                return pack("CCv", $this->ptg['ptgFuncVar'], $num_args, $this->_functions[$token][0]);
            } else {
                return pack("CCv", $this->ptg['ptgFuncVarV'], $num_args, $this->_functions[$token][0]);
            }
            // <<< Added by S.Sakai
...
//            return $left_tree.$this->_convertFunction($tree['value'], $tree['right']);
            return $left_tree.$this->_convertFunction($tree['value'], $tree['right'], $tree['force_reference']);
            // <<< Added by S.Sakai
		} else {
