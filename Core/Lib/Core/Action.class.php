...
abstract class Action {
   ...
   /**
     ...
     * @return void
     */
    protected function success($message,$jumpUrl='',$ajax=false,$parent =false,$parent_level =1) {
        $this->dispatchJump($message,1,$jumpUrl,$ajax,$parent,$parent_level);
    }
     ...
     * @return void
     */
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false,$parent =false,$parent_level =1) {
        ...
        if($status) { //发送成功信息
            ...
            // add By Richer 于20170419 为了使成功后跳转到父类页面
            if($parent) $this->assign("parent",1);
            // add By Richer 于20170419 为了使成功后跳转到父类页面
            $this->assign("parent_level",$parent_level);
