...
function encrypt($str){
	...
	return base64_encode(keyProcess($tmp,$key));
}
...
function decrypt($str){
	$key = keyProduce();
	$str=keyProcess(base64_decode($str),$key);
	$tmp='';
	for($i=0;$i<strlen($str);$i++){
		$md5=$str[$i];
		$tmp.=$str[++$i] ^ $md5;
 ...
 * 对key进行加密处理
*/
function keyProcess($str,$encrypt_key){
 ...
 */
function encrypt_php($decrypted, $password, $salt='!kQm*fG2xRq3Fgn%2') {
 ...
 * @return boolean|string
 */
function decrypt_php($encrypted, $password, $salt='!kQm*fG2xRq3Fgn%2') {
	// Build a 256-bit $key which is a SHA256 hash of $salt and $password.
	$key = hash('SHA256', $salt . $password, true);
