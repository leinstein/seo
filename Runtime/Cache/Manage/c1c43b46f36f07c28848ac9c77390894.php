<?php if (!defined('THINK_PATH')) exit();?>	
	...
	<?php if(($usertype) != "seller"): ?><style type="text/css">
	...
	<blockquote class="layui-elem-quote">
		...
		累计消费：<span><?php echo (format_money($data['record']['total_consumption'])); ?></span>
		预付冻结费用：<span><?php echo (format_money($data['record']['funs_info']['initfreezefunds'])); ?></span>
    ...
    <tr>
		...
		<?php if(($usertype) != "seller"): ?><th width="70">历史数据</th>
		<th width="70">操作</th><?php endif; ?>
    </tr> 
 	</thead>
	<tbody>
 		<?php if(!empty($list['data'])): if(is_array($list['data'])): $i = 0; $__LIST__ = $list['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<?php if(($usertype) != "seller"): ?><td>

				<a href="javascript:void(0);" onclick="open_layer('查看详情','<?php echo U('history');?>/id/<?php echo ($vo['id']); ?>?returnUrl=<?php echo (urlencode($returnUrl)); ?>','50%')" class="layui-btn layui-btn-mini">查看详情</a>
			</td>
			
			<td>
				<a href="javascript:void(0);" onclick="open_layer('指定初始排名','<?php echo U('setInitRankPage');?>/id/<?php echo ($vo['id']); ?>/rank/<?php echo ($vo['initialranking']); ?>?returnUrl=<?php echo (urlencode($returnUrl)); ?>','50%')" class="layui-btn layui-btn-mini">指定初始排名</a>
			</td><?php endif; ?>
