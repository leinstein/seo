	...
	<div class="layui-tab-content">
		...
		<form class="layui-form" name="form2" method="post" action="__URL__/assignCustomer" onkeydown="if(event.keyCode==13){return false;}">
			...
			<table class="layui-table">
				...
				<tbody>
					...
					<volist name="vo['_child']" id="vo2" >
						<tr>
							...
							<td>
								<a href="javascript:;" onclick="open_layer('用户详情','{:U('UserManagement/detail')}/id/{$vo['id']}','50%')">{$vo2['username']}</a>					
							</td>
							<td>{$vo2['usertype_desc']}</td>
							<td>{$vo2['mobileno']}</td>
							<td>{$vo2['epname']}</td>
						...
						<tr>
							...
							<td>
								<a href="javascript:;" onclick="open_layer('用户详情','{:U('UserManagement/detail')}/id/{$vo['id']}','50%')">{$vo3['username']}</a>					
							</td>
							<td>{$vo3['usertype_desc']}</td>
							<td>{$vo3['mobileno']}</td>
							<td>{$vo3['epname']}</td>
				...
				<else/>
				<tr>
					<td colspan="15" align="center">暂无相关数据</td>
				</tr>
				</notempty>
			 	</tbody>
			</table>
