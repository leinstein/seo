...
//加载
		...
		$(function(){
			...
			<elseif condition="$authtime lt date('Y-m-d')"/>
				var dialogwidth = '500px';
	            var dialogheight = '240px';
			<elseif condition="($Talentinfos['count'] eq 0) OR ($Talentinfos['count'] gt C('EXPORT_NUM')) " />
				dialogwidth = '500';
		        dialogheight = '150px';
		        contentt = "__GROUP__/TransferTips/limitTips/num/{$Talentinfos['count']}";
			</if>
	...
	<div class="wrapper">	
		...
		<div  class="ui-grid-row">
			<div class="ui-grid-25">
				<h2 class="ui-page-title">所有科技人才项目</h2>	
			</div>
		</div>
		<!-- 顶部栏目导航 end -->
			...
			<div class="ui-grid-20">
				...
				<div class="ui-box  shadow">
					...
					<div class="ui-box-container">
						...
						<input type="hidden" name="m" value="Talentinfo"/>           
							...
							<li class="ui-list-item">
								...
								<div class="ui-list-item-body item-2-line" style="height:102px;">
									<div class="param-cont">
										...
										<volist name="talenparameter['p01']" id="v">
											<a href="__URL__/index{$query_params}/p01/{$v.quotavalue}" 
												class="item-param p01single <if condition="strstr($_GET['p01'],$v['quotavalue'])">selected</if>">{$v.quotavalue}</a>
											<a class="item-param p01multiselect" style="display:none"><input type="checkbox" id="{$v.quotavalue}" name="p01[]" value="{$v.quotavalue}"><label for="{$v.quotavalue}">{$v.quotavalue}</label></a>
										</volist>
										<input type="submit" id="p01multisubmit" class="ui-button ui-button-swhite ui-button-vs" style="display:none" value="确定">
										<input type="button" id="p01multireset" class="ui-button ui-button-swhite ui-button-vs" style="display:none" value="取消">
									</div> 
								</div>
								<div class="ui-list-item-tail p01single" id="p01multi-btn"><a href="javascript:;" class="multi-btn">多选</a></div>
							</li>
							<li class="ui-list-item">
								...
								<div class="ui-list-item-body">
									<div class="param-cont">
										...
										<volist name="talenparameter['p02']" id="v">
							...
							<li class="ui-list-item last">
								<div class="param-container">
									<div class="param-cont">
											...
											<label class="param-label">人才名称：</label>
											<input type="text" class="ui-input-small" name="t3" value="{$_GET['t3']}"/>
											<label class="param-label">项目名称：</label>
											<input type="text" class="ui-input-small" name="t4" value="{$_GET['t4']}"/>
			    ...
			    <div class="ui-banner">
			    	...
			    	<span class="ui-banner-text">共有 <span class="c_red b">{$Talentinfos['count']}</span> 个符合条件的结果<eq name="searchepdir" value="true">(您选择了包含企业属性的搜索条件，系统将会过滤掉没有关联企业的数据)</eq> ，清单如下</span> 
				...
				<div class="ui-box mt10  shadow">
				    <div class="ui-box-head">
				        <span class="ui-box-head-text">排序：<a  href="__URL__/index{$query_params}/ord/%/" class="ml5 <if condition="$_GET['ord']==''||$_GET['ord']=='%'">b</if>">默认排序</a> 
				        							        <a href="__URL__/index{$query_params}/ord/year/" class="ml10 <eq name="Think.get.ord" value="year">b</eq>">认定年度</a>  </span>
				    ...
				    <table class="ui-table ui-table-follow">
				        <thead>
				            <tr>
				                ...
				                <th width="35%">企业名称</th>
				                <th width="9%">认定年度</th>
				                <th width="40%">人才类型/人才子类</th>
				                <th width="6%">操作</th>
				            </tr>
				        </thead><!-- 表头可选 -->
				        <tbody>
				        	<volist name="Talentinfos['data']" id="vo" mod="2">
				        	<eq name="mod" value="0">
				            <tr>
				            <else/>
				            <tr class="ui-table-split" style="z-index:2;">
				                ...
				                <td>{$vo.epname}</td>
				                <td>{$vo.year}</td>
				                <td>{$vo.projecttype}<br/>{$vo.projectclass}</td>
