...
<script>
		...
		$(function(){
			...
			<elseif condition="$authtime lt date('Y-m-d')"/>
				var dialogwidth = '500px';
	            var dialogheight = '240px';
			<elseif condition="($SupProjects['count'] eq 0) OR ($SupProjects['count'] gt C('EXPORT_NUM')) " />
				dialogwidth = '500';
		        dialogheight = '150px';
		        contentt = "__GROUP__/TransferTips/limitTips/num/{$SupProjects['count']}";
			</if>
	...
	<div class="wrapper">	
		...
		<div  class="ui-grid-row">
			<div class="ui-grid-25">
				<h2 class="ui-page-title">所有科技项目</h2>	
			</div>
		</div>
		<!-- 顶部栏目导航 end -->
		...
		<div class="ui-grid-row">
			...
			<div class="ui-grid-20">
					...
					<div class="ui-box-container">
						<ul class="ui-list ui-list-query">
							<li class="ui-list-item" style="z-index:1;">
								<div class="ui-list-item-head">项目类型：</div>
								<div class="ui-list-item-body">
									<div class="param-cont">
											{:w('BizcateChoose', array('queryfield'=>'prjtype', 'urlbefore'=>__URL__.'/index'.$query_params,'default' =>'选择项目类型','treename' =>'科技项目v1','rootnodepath' =>''))}
									</div> 
								</div>
								<div class="ui-list-item-tail"></div>
							</li>
							<li class="ui-list-item">
								<div class="ui-list-item-head">园区已配套金额：</div>
								<div class="ui-list-item-body">
									<div class="param-cont">
										<a href="__URL__/index{$query_params}/p01/%/" class="item-param <if condition="$_GET['p01']==''||$_GET['p01']=='%'">selected</if>">不限</a>
										<volist name="pjtparameter['p01']" id="v">
								...
								<div class="param-container">
									<div class="param-cont">
										<form name="form" method="get" action="__URL__">
											<input type="hidden" name="m" value="SupProject"/>
											...
											<label class="param-label">项目子类： </label>
				...
				<!-- 信息提示 begin  -->
			    <div class="ui-banner">
			    	<span class="ui-banner-text">共有 <span class="c_red b">{$SupProjects['count']}</span> 个符合条件的结果 <eq name="searchepdir" value="true">(您选择了包含企业属性的搜索条件，系统将会过滤掉没有关联企业的数据)</eq>，清单如下</span> 
				...
				<div class="ui-box mt10 shadow">
				    <div class="ui-box-head">
				        <span class="ui-box-head-text">排序：<a  href="__URL__/index{$query_params}/ord/%/" class="ml5 <if condition="$_GET['ord']==''||$_GET['ord']=='%'">b</if>">默认排序</a> 
				        							        <a href="__URL__/index{$query_params}/ord/year/" class="ml10 <eq name="Think.get.ord" value="year">b</eq>">立项年度</a>  </span>
				...
				<div class="ui-table-container shadow">
				    <table class="ui-table ui-table-follow">
				        <thead>
				            <tr>
				                <th width="28%">项目名称/企业名称</th>
				                <th width="9%">立项年度</th>
				                <th width="33%">项目类型/项目子类</th>
				                <th width="15%">园区已配套金额</th>
				                <th width="9%">项目状态</th>
				                <th width="6%">操作</th>
				            </tr>
				        </thead><!-- 表头可选 -->
				        <tbody>
				        	<volist name="SupProjects['data']" id="vo" mod="2">
				        	<eq name="mod" value="0">
				            <tr>
				            <else/>
				            <tr class="ui-table-split" style="z-index:2;">
				            </eq>
				                <td>{$vo.projectname}<br/>
				                <span class="b">{$vo.epname}</span></td>
				                <td>{$vo.year}</td>
				                <td>{$vo.projecttype}<br/>{$vo.subclass}</td>
			                    <td>
				                {$vo.parkhavematchtotal|format_money1} </td>
				                <td>{$vo.pjstatus} </td>
