...
(function($)
{
	jQuery.fn.multiselect2side = function (o) {
		...
		return this.each(function () {
			...
			if (height){
				var heightstyle="style='height:"+height+";'";
			}
			...
			var htmlToAdd = 
				"<div class='ms2side__div'>" +
					...
					"<div class='ms2side__select'>" +
						...
						"<select "+(heightstyle ? heightstyle :"")+" title='" + o.labelsx + "' name='" + nameSx + "' id='" + nameSx + "' size='" + size + "' multiple='multiple' ></select>" +
					"</div>" +
					...
					"<div class='ms2side__select'>" +
						...
						"<select "+(heightstyle ? heightstyle :"")+" title='" + o.labeldx + "' name='" + nameDx + "' id='" + nameDx + "' size='" + size + "' multiple='multiple' ></select>" +
					"</div>" +
					((o.selectedPosition == 'right' && o.moveOptions) ? divUpDown : "") +
				"</div>";
			...
			rightSel.dblclick(function () {
				$(this).find("option:selected").each(function(i, selected){
					if( $(this).attr("class") =="data-disabled"){
						alert("企业名称是您必选的字段！");
					}else{
				...
				if (!$(this).hasClass("ms2side__hide")) {
					if ($(this).hasClass("AddOne")) {
						...
						rightSel.find("option:selected").each(function(i, selected){
							if( $(selected).attr("class") == "data-disabled"){
								alert("企业名称是您必选的字段！");
							}else{
								$(this).remove().appendTo(leftSel);
								el.find("[value=" + $(selected).val() + "]").attr("selected", false).remove().appendTo(el);
						...
						var selecedArray = new Array();
						rightSel.find("option").each(function(i, selected){
							if( $(selected).attr("class") =="data-disabled"){
								alert("企业名称是您必选的字段！");
							}else{
								selecedArray.push(selected);
							}
						});
						$(selecedArray).appendTo(leftSel);
