	...
	layui.use(['form'], function(){
		var $ = layui.jquery, form = layui.form;
	  	...
	  	form.on('checkbox(allChoose)', function(data){
	    	var child = $(data.elem).parents('table').find('input[name="ids[]"]');
			...
			<table class="layui-table">
			    ...
			    <tr>
			      	<tr>
						...
						<th>企业名称</th>
						<!-- <th>添加日期</th> -->
						<th>所属{$list['caption1']}</th>
						<th>所属{$list['caption2']}</th>
						<!-- <th>{$list['seller_title']}</th> -->
						<!-- <th width="140">操作</th> -->
					</tr>
			 	</thead>
				<tbody>
				    ...
				    <tr>
						<td>
							<!-- 如果当前是解冻操作 -->
							<input type="checkbox" id="id_{$vo['epid']}" name="ids[]" value="{$vo['epid']}" lay-skin="primary" <eq name="vo.is_checked" value="1"> checked =""</eq>>					
							&nbsp;{$key+1}
						</td>
						<!-- 关键词 -->
						<td>
