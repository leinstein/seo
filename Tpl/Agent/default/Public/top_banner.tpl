...
<div class="ui-header">
	...
	<div class="ui-header-logo fl">
    	<!-- <a class="logo" href="{:U('Agent/Home/home')}">
        <img src="{$logo_path|default='__PUBLIC__/img/logo-white.png'}" alt="">
      	</a> -->
	<div style="font-size:25px;text-align:center;line-height:63px;font-weight:bold;color:#fff;"><i class="iconfont">&#xe62f;</i>		<if condition="$LoginUserName eq '上海潮牛'">
			潮牛网络
			<else />
            {$LoginUserName|title_show}
		</if></div>
	</div>
	<div class="ui-header-main ta_r">
		...
		<div class="fl">
			...
			<a href="{:U('Agent/Home/home')}" class="header-link <if condition="in_array(MODULE_NAME,$home_arra)">actived</if>">
			...
			<a href="{:U('Agent/Workorder/index')}"  class="header-link <if condition="MODULE_NAME == 'Workorder'">actived</if>">
				<i class="iconfont">&#xe6aa;</i>工单<gt name="untreated_workorder_num" value="0"><span class="badge">{$untreated_workorder_num}</span></gt>
			</a>
			<a href="{:U('Agent/Question/index')}"  class="header-link <if condition="MODULE_NAME == 'Question'">actived</if>">
